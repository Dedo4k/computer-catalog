        ...
        <div class="collapse navbar-collapse" id="main_nav">
            <ul class="navbar-nav">
                ...
                <li class="nav-item"><a class="nav-link"
                                        href="<c:url value="/catalog/rams"/>"><spring:message
                        code="label.components.rams"/></a></li>
...
<div class="container align-items-center mt-5">
    <form action="<c:url value="/catalog/ram/${ram.id}/comment"/>" method="post">
    ...
    <c:if test="${!ram.reviews.isEmpty()}">
        <c:forEach items="${ram.reviews}" var="review">
            <div class="card mt-1">
                <div class="card-header">
                    <div class="row">
                        <h3 class="col-11">${review.user.firstName} ${review.user.lastName}</h3>
                        <form action="/catalog/ram/${ram.id}/comment/${review.id}" method="post"
                              class="col-1">
                            <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/>
                            <sec:authorize access="hasAuthority('ADMIN')">
